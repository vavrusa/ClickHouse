    ...
    const Block & sample, const Context & context, size_t max_block_size) const
{
    ...
    else if (name == "CapnProto")
    {
        ...
        auto schema_and_root = settings.schema.toString();
        boost::split(tokens, schema_and_root, boost::is_any_of(":"));
        if (tokens.size() != 2)
            throw Exception("Format CapnProto requires capnp_schema setting to have schema_file:root_object format, e.g. 'schema.capnp:Message'");
